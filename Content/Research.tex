...
The term describes the ease of performing maintenance tasks on software, the latter of which is defined in \citet{Bray1997} :
...
It is a highly subjective term, \citet{Bray1997} mention coefficients like  average extended cyclomatic complexity per module, average number of lines of code per module and average lines of comments per module for a \textit{maintainability index} - an attempt at quantizing maintainability.
...
A definition by \citet{Bass2015}:
...
\end{itemize}
And will also be considered in this work, as adhering to above rules increases productivity of developers and lowers the barrier of entry for new coders.
...
Since \gls{ci} needs an always-on system in a consistent configuration for each of the target systems that are to be supported, that is always ready to build the code once a change it committed to a \gls{vcs}.
This technique relies on virtualization through solutions like docker\furl{https://www.docker.com/} and VMWare\furl{https://www.vmware.com/}.
...
Travis \gls{ci}\furl{https://travis-ci.org} at the time of writing provides virtual machines running Linux (Ubuntu 12.04 and 14.04) and Mac OS X to provide builds on their cloud platform.
...
Research into this field was mostly motivated by a blog post\furl{https://medium.com/@fatboyxpc/the-hard-truth-nobody-has-time-to-write-tests-68a122a1a0e3}, closing with the following words
\begin{quote}
...
A definition of extensibility in \citet{Johansson}
\begin{definition}
